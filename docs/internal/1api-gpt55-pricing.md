...
| `opus-4.8-thinking`   |                 `$5.00` |                        `$0.50` |                 `$30.00` | `p * 1.2 + c * 6` |
...
| -----: | ---------: | ----------------------: | -----------: | -----: |
|  1,000 |        100 |                   1,100 |        1,600 |   +500 |
|  1,000 |        500 |                   1,500 |        4,000 | +2,500 |
|  1,000 |      1,000 |                   2,000 |        7,000 | +5,000 |
...
billing_setting.billing_expr = {
  ...
  "opus-4.8": "tier(\"openai_price_gpt55\", p * 2.4 + c * 12)",
  "opus-4.8-thinking": "tier(\"openai_price_gpt55\", p * 2.4 + c * 12)",
  "claude-opus-4.8": "tier(\"openai_price_gpt55\", p * 2.4 + c * 12)",
  "claude-opus-4.8-thinking": "tier(\"openai_price_gpt55\", p * 2.4 + c * 12)",
...
`gpt-5.5`/`gpt-5.5-xhigh` và `opus-4.8`/`opus-4.8-thinking`/`claude-opus-4.8`/`claude-opus-4.8-thinking` hiện chốt input 1.2x, output 6x. Các model chưa có bảng giá nguồn/chiến lược pricing đã chốt (`minimax-m3`, `glm-5.2`, `glm-5.1`) tạm giữ nguyên.

### Runtime target 2026-07-01

GPT-5.5/Claude current input target mới là `1.2x`. Migration mới cần được apply riêng vào runtime trước khi coi production đã đổi.

| Surface | Runtime state |
| --- | --- |
| Frontend image | Build/deploy a new image after frontend verification; previous image still contains input 1x copy |
| Kubernetes deployment | Not changed by this file update; deploy only after explicit approval |
| DB migration | New target migration: `src/deploy/migration-2026-07-01-gpt55-claude48-input-1_2x.sql`; apply manually to `new-api-postgres` |
| Runtime billing expr | Target: GPT 5.5 and current Claude/Opus aliases = `tier("openai_price_gpt55", p * 2.4 + c * 12)` |
| Public portal | Target asset should contain `Input 1.2x · Output 6x` after frontend build/deploy |
...
select
  ...
  quota,
  case
    when model_name like 'gpt-5.5%' then round(prompt_tokens * 1.2 + completion_tokens * 6.0)
    when model_name in ('opus-4.8','opus-4.8-thinking','claude-opus-4.8','claude-opus-4.8-thinking') then round(prompt_tokens * 1.2 + completion_tokens * 6.0)
...
- DB migrations:
  ...
  - `src/deploy/migration-2026-07-01-gpt55-claude48-input-1_2x.sql` — current target GPT-5.5 and Claude/Opus 4.8 input 1.2x override.
